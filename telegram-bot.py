...
#OpenAI API key
aienv = os.getenv('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')
if aienv == None:
...
def runn(bot, update):
    ...
    global botname
    global username
    if "Ianbot" in update.message.text:
        try:
            string = update.message.text
            charout = string.split("Ianbot ",1)[1]
            botname = charout
        ...
        return
    if "@doggy2369bot " in update.message.text:
        try:
            string = update.message.text
            userout = string.split("@doggy2369bot ",1)[1]
            username = userout
